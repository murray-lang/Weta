...
void WETAFUNCATTR 
hw_servo_select(Servos* servos, uint8_t select)
{
	int8_t i;
	for (i = 0; i < servos->n_servos; i++)
		servos->servos[i].selected = (select & (1 << i)) != 0;
}
...
hw_servo_update(Servos* servos)
{
	...
	for (i = 0; i < servos->n_servos; i++)
	{
		if (servos->servos[i].selected)
		{
			...
			hw_pwm_set_duty(servos->servos[i].pwm, (WetaPwmDuty)calc_duty(&servos->servos[i]));
		}
	}
}
