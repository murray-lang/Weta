...
#include "lib/lib_comm.h"
#include "lib/lib_math.h"
#include "lib/lib_ui.h"
...
// Load configuration from flash if one is there
bool weta_run_user_config(Weta *pWeta);
...
weta_init(Weta *pWeta, Hardware *pHardware, uint16_t flags)
{
    ...
    if (MAX_UARTS == 0) {
        ...
        pWeta->sport = &pHardware->sports[0];
        pWeta->debugsport = pWeta->sport;
    } else {
        pWeta->sport = &pHardware->sports[WETA_SPORT];
        pWeta->debugsport = &pHardware->sports[WETA_DEBUG_SPORT];
    }
...
weta_open_store(Weta *pWeta, WetaStorage storage)
{
    ...
    } else if (storage == STORAGE_CONFIG_FLASH) {
        pWeta->states.configuring = true;
        return weta_store_open_flash(pWeta->hal->configFlash);
...
weta_close_store(Weta *pWeta)
{
    if (pWeta->store) {
        weta_store_close(pWeta->store);
        pWeta->store = 0;
        pWeta->states.configuring = false;
    }
}
...
{
    // If the program being rewound is a configuration then close the
    // current (config) store. Don't want to run it again
    if (pWeta->states.configuring) {
        weta_close_store(pWeta);
        pWeta->states.configuring = false;
    }

#ifdef SUPPORT_MOTORS
    hw_motor_all_off(pWeta->hal);
#endif
#ifdef SUPPORT_STEPPERS
    hw_stepper_all_off(pWeta->hal);
#endif
...
bool WETAFUNCATTR
weta_run_user_config(Weta *pWeta)
{
    PSTORE config = weta_open_store(pWeta, STORAGE_CONFIG_FLASH);
    if (!config) {
        return false;
    }

    bool ok = weta_start(pWeta, config, 0);
    if (!ok) {
        weta_close_store(pWeta);
    }
        // This will be cleared by weta_start()
    pWeta->states.configuring = true;
    return ok;
}
...
clear_run_button(Weta *pWeta)
{
    ...
    hw_gpio_clear_debounce(pWeta->hal, GPIO_INDEX_RUN);
#endif
}
...
{
    hw_gpio_debounce(pWeta->hal);
    /*
	GpioPin* run = &pWeta->hal->gpio[GPIO_INDEX_RUN];
...
#ifdef    SUPPORT_STEPPERS
    case STEPPER:
        if (hw_stepper_tick(pWeta->hal)) {
                // Set state back from STEPPER
            pWeta->states.machineState = WETA_RUN;
        }
        break;
#endif
...
exec_loop_body(Weta *pWeta)
{
    ...
    if (pWeta->states.runRequest == RUNNING) {
        ...
        case OP_RESTART:
            DEBUGMSG("OP_RESTART - %s configuring\r\n", pWeta->states.configuring ? "" : "not");
            weta_start(pWeta, 0, 0);
            break;
        case OP_IO:
            lib_io_handler(pWeta);
            break;

        case OP_MATH:
            lib_math_handler(pWeta);
            break;

        case OP_COMM:
            lib_comm_handler(pWeta);
            break;

        case OP_UI:
            lib_ui_handler(pWeta);
            break;
        ...
        case OP_UINT16: {
            ...
            if (weta_store_read_uint16(pWeta->store, pWeta->regs.pc, &value) == sizeof(value)) {
                //DEBUGMSG("weta_store_read_uint16(%d) = %d\r\n", pWeta->regs.pc, value);
        ...
        case OP_LOCAL: {
            ...
            if (weta_store_read_uint16(pWeta->store, pWeta->regs.pc, (uint16_t *) &varOffset) == sizeof(uint16_t)) {
                weta_stack_pushStackPtr(
                    pWeta->stack, (WetaStackPtr) pWeta->regs.localFrame.top + varOffset);
        ...
        case OP_PARAM: {
            ...
            if (weta_store_read_uint16(pWeta->store, pWeta->regs.pc, (uint16_t *) &varOffset) == sizeof(uint16_t)) {
                ...
                weta_stack_pushStackPtr(
                    pWeta->stack, getArgsLocation(&pWeta->regs)
        ...
        case OP_BLOCK: {
            ...
            weta_stack_pushCodePtr(
                pWeta->stack, (WetaCodePtr) pWeta->regs.pc + sizeof(uint16_t));
        ...
        case OP_TIMER:
            ...
            weta_stack_pushUint16(
                pWeta->stack, (uint16_t) ((hw_time_now() - pWeta->timerStart) / MILLIS_PER_TICK));
            break;
        ...
        case OP_AIN7: {
            int16_t value = 0;
            weta_adc_get(pWeta->hal, pWeta->regs.opCode - OP_AIN0, &value);
        ...
        case OP_AOUT7: {
            ...
            weta_dac_set(pWeta->hal, pWeta->regs.opCode - OP_AOUT0, (uint16_t) value);
        }
            break;
        case OP_AOUT: {
            uint8_t output;
        ...
        case OP_DIN7: {
            ...
            weta_digital_in(
                pWeta->hal, pWeta->regs.opCode - (uint8_t) OP_DIN0, &value
            );
        ...
        case OP_DOUT7: {
            uint8_t value;
            weta_stack_popUint8(pWeta->stack, &value);
            weta_digital_out(pWeta->hal, pWeta->regs.opCode - (uint8_t) OP_DOUT0, value);
        }
            break;
        case OP_DOUT: {
            uint8_t output;
        ...
        case OP_XOR: {
            ...
            weta_stack_pushUint8(
                pWeta->stack, (uint8_t) ((!!lhs && !rhs) || (!lhs && !!rhs))
            );
        }
            break;
        case OP_BTOS: {
...
with_current_type(Weta *pWeta)
{
    ...
    case OP_WITHINT8:
        return with_int8(pWeta);
    case OP_WITHUINT8:
        return with_uint8(pWeta);
    case OP_WITHINT16:
        return with_int16(pWeta);
    case OP_WITHUINT16:
        return with_uint16(pWeta);
    case OP_WITHBOOL:
        return with_bool(pWeta);
    case OP_WITHPTR:
        return with_stackptr(pWeta);
#ifdef SUPPORT_32BIT
    case OP_WITHINT32:
        return with_int32(pWeta);
    case OP_WITHUINT32:
        return with_uint32(pWeta);
#endif
#ifdef SUPPORT_FLOAT
    case OP_WITHFLOAT:
        return with_float(pWeta);
#endif
#ifdef SUPPORT_DOUBLE
    case OP_WITHDOUBLE:
        return with_double(pWeta);
#endif
#ifdef SUPPORT_STRING
    case OP_WITHSTRING:
        return with_string(pWeta);
#endif
...
{
    //pWeta->states.machineState = WETA_EXT_CONFIG;
    ...
    //DEBUGMSG("Opened store for write\r\n");
    ...
    char *cursor = weta_strchr(jsonArray, '[');
    cursor++;    // Step past the '['
    char *endptr = cursor;
    long nextCode;
    bool error = false;
    ...
    while (*endptr != ']') {
        ...
        DEBUGMSG("%ld\n\r", nextCode);
        if (weta_store_write_byte(store, (uint8_t) nextCode) == 0) {
            DEBUGMSG("weta_store_write_byte() failed\r\n");
            error = true;
            break;
        }
    ...
    if (error) {
        DEBUGMSG("Error writing codes.\r\n");
        weta_store_close(store);
        return false;
    } else if (!weta_store_flush(store)) {
        DEBUGMSG("weta_store_flush() failed.\r\n");
        weta_store_close(store);
        return false;
    }
