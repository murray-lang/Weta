...
const byteTimeout        = 200;
const cmdSetStartAddress = 131;
const cmdWriteBytes      = 133;

var portConfig = { baudrate: 9600, autoOpen: false };
...
function CricketProgrammer(device, normalCallback, errorCallback)
{
	...
	[
        ...
        //this.sendRun.bind(this),
        //this.writeCallback.bind(this),
        //this.wait.bind(this, connectTimeout),
        //this.verifySendRunReply.bind(this),
...
CricketProgrammer.prototype.setData =
	function(data)
	{
		...
		this.data = new Uint8Array(data.codes);
		this.address = data.address === undefined ? 0 : data.address;
	};
...
CricketProgrammer.prototype.connectSerial =
	function()
	{
		...
		try
		{
            this.serialPort = new SerialPort(this.device, portConfig);
...
CricketProgrammer.prototype.flushReceivedData =
    function()
    {
        this.serialPort.flush(this.sequence.getNext());

    };

CricketProgrammer.prototype.flushCallback =
    function(error)
    {
        if (error)
...
CricketProgrammer.prototype.writeStartAddress =
	function() 
	{
		...
		try
		{
			var buf = bytesToArrayBuffer([cmdSetStartAddress, (this.address >> 8) & 0xff , this.address & 0xff]);
			this.serialPort.write(buf, this.sequence.getNext());
		}
		catch (err)
		{
	...
	function()
	{
		...
		try
		{
			...
			if (   reply[0] != cmdSetStartAddress 
				|| reply[1] != (this.address >> 8) & 0xff
				|| reply[2] != this.address & 0xff)
			{
...
CricketProgrammer.prototype.sendRun =
    function()
    {
        //log("sendRun()");
        try
        {
            var buf = bytesToArrayBuffer([0x86]);
            this.serialPort.write(buf, this.sequence.getNext());
        }
        catch (err)
        {
            this.cleanup("Error sending run - " + err.message);
        }
    };

CricketProgrammer.prototype.verifySendRunReply =
    function(readInfo)
    {
        //log("verifySendRunReply()");
        this.sequence.next();
    };
...
CricketProgrammer.prototype.done =
    function()
    {
        console.log("Done.");
        process.exit(0);
...
if (process.argv.length > 2)
{
    ...
    try
    {
        wasmText = fs.readFileSync(codeFileName, { encoding: 'utf8' });
        wasmJsonObj = JSON.parse(wasmText);
    }
    catch (e)
    {
    ...
    if (wasmJsonObj.codes === undefined)
    {
        console.error("No 'codes' attribute in JSON file " + codeFileName + ".");
        process.exit(1);
    }
    console.log("Sending byte codes in file " + codeFileName + " to " + serialPort);

    var bp = new CricketProgrammer(serialPort, normalCallback, errorCallback);
    try
    {
        //console.log(codes);
        bp.run(wasmJsonObj);
