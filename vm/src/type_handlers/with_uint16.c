...
min_uint16(uint16_t lhs, uint16_t rhs)
{
	return lhs <= rhs ? lhs: rhs;
}

uint16_t WETAFUNCATTR 
max_uint16(uint16_t lhs, uint16_t rhs)
{
	return lhs >= rhs ? lhs: rhs;
}

bool WETAFUNCATTR 
with_uint16(Weta* pWeta)
{
	...
	case OP_ENCODE:
		{
			...
			weta_stack_popUint8(pWeta->stack, &size);
				//Get a pointer to the array of doubles on the stack.
				// Note that the stack orders bytes in native order, so no endian
				// conversion is required yet.
			uint16_t* values = (uint16_t*)weta_stack_getStackAddress(pWeta->stack, location);
	...
	case OP_DECODE:
		{
			...
			uint8_t* bytes = (uint8_t*)weta_stack_getStackAddress(pWeta->stack, location);
				...
				// from last to first.
			int16_t i;
			for (i = size - sizeof(uint16_t); i >= 0; i -= sizeof(uint16_t))
			{
				uint16_t next = ntoh_uint16(&bytes[i]);
				weta_stack_pushUint16(pWeta->stack, next);
			}
				// Lastly, push the number of items in the list
			weta_stack_pushUint8(pWeta->stack, size / sizeof(uint16_t));
		}
	...
	case OP_LSHIFT:
		{
			...
			weta_stack_popUint16(pWeta->stack, &lhs);
			if (shift >= 0)
	...
	case OP_SET:
		...
		weta_stack_popStackPtr(pWeta->stack, &location);
		weta_stack_popUint16(pWeta->stack, &rhs);

		weta_stack_setUint16(pWeta->stack, location, rhs);
		return true;

	case OP_GET:
		//Serial.println("---get---");
		weta_stack_popStackPtr(pWeta->stack, &location);
		weta_stack_getUint16(pWeta->stack, location, &rhs);
		weta_stack_pushUint16(pWeta->stack, rhs);
		return true;
	...
	case OP_ASET:
		...
		weta_stack_popUint16(pWeta->stack, &rhs);
		weta_stack_setUint16(pWeta->stack, location + index * sizeof(uint16_t), rhs);
		return true;
	...
	case OP_AGET:
		...
		weta_stack_popStackPtr(pWeta->stack, &location);
		weta_stack_popStackPtr(pWeta->stack, &index);
		weta_stack_getUint16(pWeta->stack, location + index * sizeof(uint16_t), &rhs);
		weta_stack_pushUint16(pWeta->stack, rhs);
		return true;
	...
	case OP_OUTPUT:
		{
			//Serial.print("output ");
			weta_stack_popUint16(pWeta->stack, &rhs);	// The return value
			...
			weta_stack_setUint16(pWeta->stack, location - (WetaStackPtr)argsSize - sizeof(uint16_t), rhs);
		}
		return true;
	...
	case OP_TX:
		{
			...
			hton_uint16(rhs, nbuf);
			hw_serial_write(pWeta->sport, nbuf, sizeof(uint16_t));
		}
		return true;
	...
	case OP_RX:
		{
			//Serial.println("---rx---");
			uint8_t nbuf[sizeof(uint16_t)];
			hw_serial_read(pWeta->sport, nbuf, sizeof(uint16_t), -1);
			weta_stack_pushUint16(pWeta->stack, ntoh_uint16(nbuf));
		}
		return true;
	...
	case OP_FOREACH:
		{	
			...
			if (itemsRemaining > 0) // Any items remaining?
			{
				...
				weta_stack_setStackPtr(pWeta->stack, location, weta_stack_getTop(pWeta->stack) - sizeof(uint16_t));
			...
			{
				ascendBlock(&pWeta->regs);			// Leaving. weta_stack_pop the block.
			}
		}
		return true;
	}
	...
	case OP_MIN:
		weta_stack_pushUint16(pWeta->stack, (uint16_t)min_uint16(lhs, rhs));
		return true;	
	case OP_MAX:
		weta_stack_pushUint16(pWeta->stack, (uint16_t)max_uint16(lhs, rhs));
		return true;
	}
	return false;
}
