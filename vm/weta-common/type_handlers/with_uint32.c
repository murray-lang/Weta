...
min_uint32(uint32_t lhs, uint32_t rhs)
{
	return lhs <= rhs ? lhs: rhs;
}

uint32_t WETAFUNCATTR 
max_uint32(uint32_t lhs, uint32_t rhs)
{
	return lhs >= rhs ? lhs: rhs;
}

bool WETAFUNCATTR 
with_uint32(Weta* pWeta)
{
	...
	case OP_LSHIFT:
		{
			...
			weta_stack_popUint32(pWeta->stack, &lhs);
			if (shift >= 0)
	...
	case OP_SET:
		...
		weta_stack_setUint32(pWeta->stack, location, rhs);
		return true;

	case OP_GET:
		//Serial.println("---get---");
		weta_stack_popStackPtr(pWeta->stack, &location);
		weta_stack_getUint32(pWeta->stack, location, &rhs);
		weta_stack_pushUint32(pWeta->stack, rhs);
		return true;
	...
	case OP_ASET:
		...
		weta_stack_popUint32(pWeta->stack, &rhs);
		weta_stack_setUint32(pWeta->stack, location + index * sizeof(uint32_t), rhs);
		return true;
	...
	case OP_AGET:
		...
		weta_stack_popStackPtr(pWeta->stack, &location);
		weta_stack_popStackPtr(pWeta->stack, &index);
		weta_stack_getUint32(pWeta->stack, location + index * sizeof(uint32_t), &rhs);
		weta_stack_pushUint32(pWeta->stack, rhs);
		return true;
	...
	case OP_OUTPUT:
		{
			//Serial.print("output ");
			weta_stack_popUint32(pWeta->stack, &rhs);	// The return value
			...
			weta_stack_setUint32(pWeta->stack, location - (WetaStackPtr)argsSize - sizeof(uint32_t), rhs);
		}
		return true;
	...
	case OP_FOREACH:
		{	
			...
			if (itemsRemaining > 0)				// Any items remaining
			{
				...
				weta_stack_setStackPtr(pWeta->stack, location, weta_stack_getTop(pWeta->stack) - sizeof(uint32_t));
	...
	case OP_MIN:
		weta_stack_pushUint32(pWeta->stack, (uint32_t)min_uint32(lhs, rhs));
		return true;	
	case OP_MAX:
		weta_stack_pushUint32(pWeta->stack, (uint32_t)max_uint32(lhs, rhs));
		return true;
	}
	return false;
}
