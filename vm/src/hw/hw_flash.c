...
hw_flash_erase(PFLASH flash)
{
	...
	FlashRawResult rc = hw_flash_raw_erase_sector(flash->base >> 12);
...
{
		// This isn't necessarily true, but it's the amount we can manage with
...
{
	FlashRawResult rc;
	rc = hw_flash_raw_read(buffer->addr, &buffer->asStdInt, FLASH_ALIGN);
	return rc == FLASH_RAW_RESULT_OK;
}
